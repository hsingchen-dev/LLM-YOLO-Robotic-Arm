...
本プロジェクトの目的は、生成AI-ChatGPT、画像認識YOLO、逆運動学を組み合わせることで、ロボットアームが自然言語の命令を理解し、日本の高度な自動化システムと連携して適切な動作を実行できるようにすることです。  
...
- **田坂 風月（タサカ フヅキ））**  
  市場調査、AI技術の実装および評価を担当。  
  システム開発と社会実装の橋渡しとして、ユーザー調査やAIエージェントの動作評価、ユーザビリティテストを実施。プロトタイプ改善や社会課題の解決に向けた提案も行う。  
  *神戸大学*
...
- **孫 妍（ソン ケン）**    
孙彦 ​​​ 
  人間行動とロボットインタラクションを専門とし、エージェントによるプロンプト処理とシステムの行動ロジック設計を担当。  
  *神戸大学*
...
本システムは、従来の物体認識を超えたマルチターゲット視覚意思決定アーキテクチャを導入しています。  
従来バージョンでは単一ターゲットの識別のみが可能でしたが、本アップグレード版では、複数の検出物体を解析し、その中から把持に最も適した対象を選択することができます。

具体的には、まず YOLO モジュールがシーン内のすべての候補物体を検出し、それぞれの座標、信頼度スコア、および色情報を出力します。  
次に、これらの検出結果を、ユーザーの選好を定義した第2プロンプト（Prompt）と統合します。  
このプロンプトには、たとえば「清潔で損傷のない物体を優先する」「画像中心に近い物体を優先する」といった選択ルールや、同順位候補がある場合の判断基準が含まれます。

統合されたデータ（アノテーション付き画像、JSON形式の検出結果、Prompt）は、視覚対応の大規模言語モデル（Vision LLM）に送られます。  
Vision LLM は、画像内容とテキストルールの両方を解釈し、意味的推論と視覚的推論を組み合わせて、操作対象として最適な物体を決定します。  
その結果として、ターゲットのラベル、座標、および選択理由を含む構造化JSON応答を出力します。

このアプローチにより、本システムは複数物体が存在する場面において、人間に近い意思決定を実現します。  
すなわち、ロボットは単に物体を認識するだけでなく、状況や選好に基づいて知的に選択することが可能になります。
...
本システムの最新版 SKYNET 16 は、大規模言語モデル（LLM）を単一の end-to-end 意思決定器として用いるのではなく、複数の階層にまたがる推論モジュールとして組み込み、ロボットの行動を段階的に構造化しています。

Layer 1: 目標選択と特徴理解
まず、プロンプト設計を通じて、LLM の物体特徴識別能力を強化します。  
視覚検出結果に基づき、LLM は物体カテゴリ、外観的特徴、把持対象としての適切性を分析し、複数の候補が存在する場合でも、明確かつ説明可能な目標選択を実現します。

Layer 2: 遮蔽・障害物推論
目標決定後、システムは遮蔽推論の段階へ進みます。  
この段階では、自由な視覚推論のみに依存するのではなく、中心点被覆、相対的位置関係、バウンディングボックスの重なりといった幾何学的制約を構造化入力として導入します。  
LLM はこれらの情報をもとに残る曖昧性を解消し、対象物が障害物によって遮蔽されているか、また把持前に遮蔽物を除去すべきかを判断します。

Layer 3: 逆運動学による実行
最終的な判断が得られた後、その戦略は実際の物理動作へと変換されます。  
対象物および障害物の位置はロボット座標系へ変換され、逆運動学により関節角度を計算することで、ロボットアームは現実空間において把持動作または障害物除去動作を実行します。

特徴
このような階層設計により、LLM は高次の意味理解を徐々に具体的な物理行動へと接続し、  
**Perception → Reasoning → Decision → Execution**  
という一連のループを形成します。  
本プロジェクトは、実ロボットシステムにおける embodied intelligence 実現に向けた、実践的かつ拡張可能なアプローチを示すものです。
