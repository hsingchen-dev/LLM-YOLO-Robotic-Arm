...
本システムの最新版 SKYNET 16 は、ロボットマニピュレーションのための**階層型 Embodied Intelligence アーキテクチャ**を紹介します。  
本システムでは、大規模言語モデル（LLM）を単一の end-to-end 意思決定器として用いるのではなく、複数の階層にまたがる推論モジュールとして組み込み、ロボットの行動を段階的に構造化しています。
